...
dotenv.config();
const app = express();
function getPoolerConnectionString(url) {
...
app.post('/api/admin/orders', authenticateAdmin, async (req, res) => {
    try {
        ...
        const itemsTotal = itemsList.reduce((s, item) => s + (Number(item.price) * Number(item.quantity)), 0);
...
app.put('/api/orders/:id', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        const itemsTotal = newItems.reduce((s, i) => s + (Number(i.price) * Number(i.quantity)), 0);
        const shipping = actualShippingCost !== undefined ? Number(actualShippingCost) : existing.actualShippingCost;
        const orderTotal = itemsTotal + shipping;
...
app.get('/api/analytics/product-performance', authenticateAdmin, async (req, res) => {
    try {
        ...
        items.forEach(item => {
            ...
            if (!perfMap[key])
                perfMap[key] = { name: key, quantity: 0, revenue: 0, orders: 0 };
...
app.get('/api/customers', authenticateAdmin, async (req, res) => {
    try {
        ...
        orders.forEach(order => {
            ...
            if (order.orderNumber && (order.paymentStatus === 'Paid' || order.paymentStatus === 'Partially Paid'))
                customerMap[key].totalSpent += orderTotal;
...
app.put('/api/delivery-zones/:id', authenticateAdmin, async (req, res) => {
    const { name, pincode, delivery_charge, is_active } = req.body;
    try {
        const zone = await prisma.deliveryZone.update({ where: { id: req.params.id }, data: { name, pincode, delivery_charge: Number(delivery_charge) || 0, is_active } });
        res.json(zone);
    }
    catch (error) {
...
app.put('/api/testimonials/:id', authenticateAdmin, async (req, res) => {
    const { customer_name, location, text, rating, is_active } = req.body;
    try {
        const t = await prisma.testimonial.update({ where: { id: req.params.id }, data: { customer_name, location, text, rating: Number(rating) || 5, is_active } });
        res.json(t);
    }
    catch (error) {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (data.categories && Array.isArray(data.categories)) {
                for (const c of data.categories) {
                    ...
                    if (c.createdAt)
                        catData.createdAt = new Date(c.createdAt);
                    if (c.updatedAt)
                        catData.updatedAt = new Date(c.updatedAt);
            ...
            if (data.products && Array.isArray(data.products)) {
                for (const p of data.products) {
                    ...
                    if (p.createdAt)
                        prodData.createdAt = new Date(p.createdAt);
                    if (p.updatedAt)
                        prodData.updatedAt = new Date(p.updatedAt);
            ...
            if (data.orders && Array.isArray(data.orders)) {
                for (const o of data.orders) {
                    ...
                    if (o.createdAt)
                        ordData.createdAt = new Date(o.createdAt);
                    if (o.approvedAt)
                        ordData.approvedAt = new Date(o.approvedAt);
            ...
            if (data.expenses && Array.isArray(data.expenses)) {
                for (const e of data.expenses) {
                    ...
                    if (e.date)
                        expData.date = new Date(e.date);
                    if (e.createdAt)
                        expData.createdAt = new Date(e.createdAt);
                    if (e.updatedAt)
                        expData.updatedAt = new Date(e.updatedAt);
            ...
            if (data.testimonials && Array.isArray(data.testimonials)) {
                for (const t of data.testimonials) {
                    ...
                    if (t.createdAt)
                        testData.createdAt = new Date(t.createdAt);
                    if (t.updatedAt)
                        testData.updatedAt = new Date(t.updatedAt);
            ...
            if (data.deliveryZones && Array.isArray(data.deliveryZones)) {
                for (const dz of data.deliveryZones) {
                    ...
                    if (dz.createdAt)
                        dzData.createdAt = new Date(dz.createdAt);
                    if (dz.updatedAt)
                        dzData.updatedAt = new Date(dz.updatedAt);
            ...
            if (data.customerProfiles && Array.isArray(data.customerProfiles)) {
                for (const cp of data.customerProfiles) {
                    ...
                    if (cp.createdAt)
                        cpData.createdAt = new Date(cp.createdAt);
                    if (cp.updatedAt)
                        cpData.updatedAt = new Date(cp.updatedAt);
...
app.post('/api/products/bulk', authenticateAdmin, async (req, res) => {
    ...
    try {
        await prisma.$transaction(updates.map((prodUpdate) => {
            const prodData = {};
            ...
            const variantUpdatePromises = (prodUpdate.variants || []).map((vUpdate) => {
                const varData = {};
                if (vUpdate.variantPrice !== undefined)
                    varData.variantPrice = Number(vUpdate.variantPrice);
                if (vUpdate.costPrice !== undefined)
                    varData.costPrice = Number(vUpdate.costPrice);
                if (vUpdate.packagingCharge !== undefined)
                    varData.packagingCharge = Number(vUpdate.packagingCharge);
